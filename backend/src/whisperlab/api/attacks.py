...
async def create_attack(body: AttackCreate, db: AsyncSession = Depends(get_db)):
    attack = Attack(
...
async def list_attacks(
    ...
    result = await db.execute(query.order_by(Attack.created_at.desc()).offset(offset).limit(page_size))
    items = [AttackOut.model_validate(a) for a in result.scalars().all()]
    return PaginatedResponse(items=items, total=total, page=page, page_size=page_size, pages=pages)


@router.get("/{attack_id}", response_model=AttackOut, responses={404: {"model": ErrorResponse}})
async def get_attack(attack_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def get_attack_results(attack_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Attack).where(Attack.id == attack_id))
    attack = result.scalar_one_or_none()
    if attack is None:
        raise HTTPException(status_code=404, detail="Attack not found")
    if attack.result_json is None:
        raise HTTPException(status_code=404, detail="Attack has not completed yet")
    return {"attack_id": attack.id, "attack_type": attack.attack_type, "results": attack.result_json}
