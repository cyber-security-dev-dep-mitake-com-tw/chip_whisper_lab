...
async def register_target(body: TargetCreate, db: DbSession):
    target = Target(
        name=body.name,
...
async def get_target(target_id: UUID, db: DbSession):
...
async def flash_firmware(target_id: UUID, body: FlashRequest, db: DbSession):
    result = await db.execute(select(Target).where(Target.id == target_id))
    target = result.scalar_one_or_none()
    if target is None:
        raise HTTPException(status_code=404, detail="Target not found")
    ...
    return FlashResponse(
        target_id=target.id, firmware_path=body.firmware_path, status="flashed"
    )
...
async def test_connection(target_id: UUID, db: DbSession):
    result = await db.execute(select(Target).where(Target.id == target_id))
    target = result.scalar_one_or_none()
    if target is None:
        raise HTTPException(status_code=404, detail="Target not found")
